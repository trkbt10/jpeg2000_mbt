...
### 2026-02-26 / 外部コーパス(openjpeg-data)テストケース運用導線

- 完了要求ID: なし（Status変更なし、外部コーパス適用範囲の拡張）
- 実施内容: `tools/external_corpus_cycle.sh` を追加し、openjpeg-data リポジトリ配下の `.j2k/.j2c` を再帰走査して `default/strict/roundtrip` を検証可能化
...
- 継続課題: openjpeg-data repo 実体を指定して matrix 実行し、strict fail 一覧を別マニフェスト化（必要なら `strict-expected-failures-external.txt` を追加）
