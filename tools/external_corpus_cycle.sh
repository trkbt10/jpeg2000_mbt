...
# Validate codestream compatibility using test assets from the upstream openjpeg-data repository.
# Expected source repo: https://github.com/uclouvain/openjpeg-data

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
EXTERNAL_CORPUS_DIR="${1:-${EXTERNAL_CORPUS_DIR:-}}"
OUT_CSV="${2:-$ROOT_DIR/samples/generated/corpus/external-corpus-matrix.csv}"
...
if [ -z "$EXTERNAL_CORPUS_DIR" ]; then
  echo "usage: $0 <external_corpus_dir> [out_csv]" >&2
  exit 1
fi

if [ ! -d "$EXTERNAL_CORPUS_DIR" ]; then
  echo "external corpus directory not found: $EXTERNAL_CORPUS_DIR" >&2
  exit 1
fi
...
while IFS= read -r in_bin; do
  ...
  rel="${in_bin#$EXTERNAL_CORPUS_DIR/}"
...
done < <(find "$EXTERNAL_CORPUS_DIR" -type f \( -name '*.j2k' -o -name '*.j2c' \) | LC_ALL=C sort)

if [ "$found" -eq 0 ]; then
  echo "no .j2k/.j2c files found under $EXTERNAL_CORPUS_DIR" >&2
  exit 1
fi
...
echo "external corpus matrix summary: files=$count default_ok=$ok_default strict_ok=$ok_strict roundtrip_ok=$ok_roundtrip strict_fail=$strict_fail unexpected_strict_fail=$unexpected recovered_strict=$recovered csv=$OUT_CSV"
